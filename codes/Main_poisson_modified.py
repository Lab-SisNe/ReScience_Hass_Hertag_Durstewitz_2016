...
##### Setting simulation directory (as 'simulation_{}', with {} being the lowest non-negative integer corresponding to non-existing directory)
...
pulse2 = 2500
PoissonStimuli = [
                    [100, 1, 30, 2, 0,  pulse1, pulse1+100, [[0, 0, 0.1],],],
                    [100, 1, 30, 2, 0,  pulse2, pulse2+100, [[7, 0, 0.1],],],
                  ]
############### Regular stimuli ###############
...
if control_param['run']:
    ...
    target1 = np.asarray(cortex.group_distr[0][0])
    
    fig, ax = cortex.raster_plot(tlims=[pulse1-100, pulse1+400], save=False, figuresize=(12,10), plotsize=5)
    plt.sca(ax)
    plt.vlines(pulse1, 0, cortex.NeuPar.shape[1]+15, color='black', linestyle='dotted', linewidth=2)
    plt.vlines(pulse1+100, 0, cortex.NeuPar.shape[1]+15, color='black', linestyle='dotted', linewidth=2)
    
    plt.vlines(pulse1, min(target1), max(target1), color='purple', linestyle='--',  linewidth=5)
    plt.vlines(pulse1+100, min(target1), max(target1), color='green', linestyle='--',  linewidth=5)
    if not os.path.isdir('Figures'):
        os.mkdir('Figures')
    fig.savefig('Figures/Fig17a.png')
    
    target2 = np.asarray(cortex.group_distr[7][0])+15
    
    fig, ax = cortex.raster_plot(tlims=[pulse2-100, pulse2+400], save=False, figuresize=(12,10), plotsize=5)
    plt.sca(ax)
    plt.vlines(pulse2, 0, cortex.NeuPar.shape[1]+15, color='black', linestyle='dotted', linewidth=2)
    plt.vlines(pulse2+100, 0, cortex.NeuPar.shape[1]+15, color='black', linestyle='dotted', linewidth=2)
    
    plt.vlines(pulse2, min(target2), max(target2), color='purple', linestyle='--' , linewidth=5)
    plt.vlines(pulse2+100, min(target2), max(target2), color='green', linestyle='--' , linewidth=5)
    fig.savefig('Figures/Fig17b.png')
